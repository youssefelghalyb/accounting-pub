    ...
    <div class="space-y-6">
        ...
        <nav aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                ...
                <li>
                    <span class="text-gray-900 font-medium">{{ __('hr::deductions.deduction_reference') }}
                        #{{ $deduction->id }}</span>
                </li>
            </ol>
        </nav>
            ...
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-1">{{ __('hr::deductions.deduction_date') }}:
                            {{ $deduction->deduction_date->format('Y-m-d') }}</p>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <form action="{{ route('hr.deductions.destroy', $deduction) }}" method="POST" class="inline-block"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::employee.employee_name') }}</label>
                        <p class="text-base font-semibold text-gray-900">{{ $deduction->employee->name }}</p>
                    </div>
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::employee.employee_code') }}</label>
                        <p class="text-base font-semibold text-gray-900">{{ $deduction->employee->employee_code }}</p>
                    </div>
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::department.department') }}</label>
                        <p class="text-base font-semibold text-gray-900">{{ $deduction->employee->department->name }}
                        </p>
                    </div>
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::employee.position') }}</label>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::deductions.deduction_type') }}</label>
                        @if ($deduction->type === 'days')
                        ...
                        @else
                            <p class="text-base font-semibold text-gray-900">
                                {{ __('hr::deductions.type_unpaid_leave') }}</p>
                        @endif
                    </div>
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::deductions.deduction_date') }}</label>
                        <p class="text-base font-semibold text-gray-900">
                            {{ $deduction->deduction_date->format('Y-m-d') }}</p>
                    </div>
                    @if ($deduction->days)
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::deductions.days') }}</label>
                            <p class="text-base font-semibold text-gray-900">{{ $deduction->days }}
                                {{ __('hr::deductions.days_label') }}</p>
                        </div>
                    @endif
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-600 mb-1">{{ __('hr::deductions.amount') }}</label>
                        <p class="text-2xl font-bold text-red-600">{{ number_format($deduction->amount, 2) }}
                            {{ __('common.currency') }}</p>
                    </div>
                </div>
                        ...
                        <div class="flex items-center gap-3 p-4 bg-purple-50 rounded-lg border border-purple-200">
                            ...
                            <div>
                                <p class="text-sm font-medium text-purple-900">
                                    {{ __('hr::deductions.linked_to_leave') }}</p>
                                <p class="text-xs text-purple-700 mt-1">{{ __('hr::deductions.leave_reference') }}
                                    #{{ $deduction->leave_id }}</p>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <label
                        class="block text-sm font-medium text-gray-600 mb-2">{{ __('hr::deductions.reason') }}</label>
                ...
                @if ($deduction->notes)
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <label
                            class="block text-sm font-medium text-gray-600 mb-2">{{ __('hr::deductions.notes') }}</label>
                        <div class="bg-gray-50 rounded-lg p-4">
        ...
        <div class="flex justify-start">
            <a href="{{ route('hr.deductions.index') }}"
                ...
                <svg class="w-5 h-5 {{ app()->getLocale() == 'ar' ? 'rotate-180' : '' }}" fill="none"
