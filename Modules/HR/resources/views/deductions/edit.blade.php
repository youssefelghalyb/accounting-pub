<x-dashboard :pageTitle="__('hr::deductions.edit_deduction')">
    <div class="max-w-4xl mx-auto">
        ...
        <nav class="mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                ...
                <li>
                    <span class="text-gray-900 font-medium">{{ __('hr::deductions.edit_deduction') }}</span>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-900">{{ __('hr::deductions.edit_deduction') }}</h2>
                <p class="text-sm text-gray-600 mt-1">{{ __('common.required_fields') }}</p>
            </div>

            <div class="p-6">
                <x-dashboard.packages.form-builder 
                    :action="route('hr.deductions.update', $deduction)" 
                    method="POST"
