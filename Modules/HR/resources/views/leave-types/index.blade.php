<x-dashboard :pageTitle="__('hr::leaveTypes.leave_type_management')">
    @php
        // Prepare data
        $tableData = $leaveTypes->map(function($type) {
            return [
                'id' => $type->id,
                'name' => $type->name,
        ...
        $tableColumns = [
            ...
            [
                ...
                'render' => function($row) {
                    ...
                    return '<span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">' . __('common.unlimited') . '</span>';
                }
            ],
            [
                'label' => __('hr::leaveTypes.type'),
                'field' => 'is_paid',
                'align' => 'center',
                'render' => function($row) {
                    if ($row['is_paid']) {
                        return '<span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">' . __('hr::leaveTypes.paid') . '</span>';
                    }
                    return '<span class="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">' . __('hr::leaveTypes.unpaid') . '</span>';
                }
            ],
            [
    ...
    <div class="space-y-6">
        ...
        <x-dashboard.packages.data-table
            :title="__(key: 'hr::leaveTypes.leave_type_list')"
            :description="__('hr::leaveTypes.total_leave_types') . ': ' . $leaveTypes->count()"
            searchable
            :searchRoute="route('hr.leave-types.index')"
            :searchPlaceholder="__('hr::leaveTypes.search_leave_types')"
            :data="$tableData"
            :columns="$tableColumns"
            :actions="$tableActions"
            :createRoute="route('hr.leave-types.create')"
            :createLabel="__('hr::leaveTypes.add_leave_type')"
            :emptyStateTitle="__('hr::leaveTypes.no_leave_types')"
