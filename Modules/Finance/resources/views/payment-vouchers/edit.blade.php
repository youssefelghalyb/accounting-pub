<x-dashboard :pageTitle="__('finance::payment.edit_payment')">
    <div class="max-w-4xl mx-auto">
        ...
        <nav class="mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                ...
                <li>
                    <span class="text-gray-900 font-medium">{{ __('finance::payment.edit_payment') }} - {{ $paymentVoucher->voucher_number }}</span>
                </li>
            </ol>
        </nav>

        <form action="{{ route('finance.payment-vouchers.update', $paymentVoucher) }}" method="POST" id="paymentForm">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-600 mt-1">{{ $paymentVoucher->voucher_number }}</p>
                    </div>
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div id="invoiceInfo" class="mt-2 {{ $paymentVoucher->purchase_invoice_id ? '' : 'hidden' }}">
                                    <p class="text-sm text-gray-600">
                                        ...
                                        <span id="outstandingAmount" class="font-bold text-red-600">
                                            {{ $paymentVoucher->purchaseInvoice ? number_format($paymentVoucher->purchaseInvoice->outstanding_balance, 2) : '' }}
                                        </span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="voucher_date" value="{{ old('voucher_date', $paymentVoucher->voucher_date->format('Y-m-d')) }}" required
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                                @error('voucher_date')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="amount" id="amount" step="0.01" min="0.01" value="{{ old('amount', $paymentVoucher->amount) }}" required
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                                @error('amount')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="account_id" required
                                    ...
                                    @foreach($accounts as $account)
                                        <option value="{{ $account->id }}" {{ $paymentVoucher->account_id == $account->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="payment_method" id="payment_method" required
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                                    <option value="cash" {{ $paymentVoucher->payment_method == 'cash' ? 'selected' : '' }}>{{ __('finance::payment.method_cash') }}</option>
                                    <option value="cheque" {{ $paymentVoucher->payment_method == 'cheque' ? 'selected' : '' }}>{{ __('finance::payment.method_cheque') }}</option>
                                    <option value="bank_transfer" {{ $paymentVoucher->payment_method == 'bank_transfer' ? 'selected' : '' }}>{{ __('finance::payment.method_bank_transfer') }}</option>
                                    <option value="credit_card" {{ $paymentVoucher->payment_method == 'credit_card' ? 'selected' : '' }}>{{ __('finance::payment.method_credit_card') }}</option>
                                    <option value="other" {{ $paymentVoucher->payment_method == 'other' ? 'selected' : '' }}>{{ __('finance::payment.method_other') }}</option>
                                </select>
                            ...
                            <div id="chequeDetails" class="md:col-span-2 {{ $paymentVoucher->payment_method == 'cheque' ? '' : 'hidden' }}">
                                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" name="cheque_number" id="cheque_number" value="{{ old('cheque_number', $paymentVoucher->cheque_number) }}"
                                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                            ...
                                            <input type="date" name="cheque_date" id="cheque_date" value="{{ old('cheque_date', $paymentVoucher->cheque_date?->format('Y-m-d')) }}"
                                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="transaction_reference" value="{{ old('transaction_reference', $paymentVoucher->transaction_reference) }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" rows="3" placeholder="{{ __('finance::payment.enter_description') }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('description', $paymentVoucher->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <button type="submit"
                            ...
                            {{ __('finance::payment.update_payment') }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            partySelect.addEventListener('change', function() {
                ...
                const currentInvoiceId = {{ $paymentVoucher->purchase_invoice_id ?? 'null' }};
                ...
                if (partyId) {
                    fetch(`/finance/parties/${partyId}/purchase-invoices`)
                        ...
                        .then(invoices => {
                            ...
                            // Trigger change if there was a selected invoice
                            if (currentInvoiceId) {
