...
<head>
    ...
    <title>{{ __('finance::payment.payment_voucher') }} #{{ $paymentVoucher->voucher_number }}</title>
    ...
    <div class="p-4">
        <div class="receipt-container shadow-lg">
            <div class="p-6">
                    ...
                    <div class="text-{{ $printLang == 'ar' ? 'left' : 'right' }}">
                        <div class="border-2 border-black px-3 py-1 inline-block mb-2">
                            <h2 class="text-14 font-bold">
                                {{ $printLang == 'ar' ? 'سند صرف' : 'PAYMENT VOUCHER' }}
                            </h2>
                        </div>
                        <table class="text-9" style="margin-{{ $printLang == 'ar' ? 'right' : 'left' }}: auto;">
                            <tr>
                                ...
                                <td class="font-bold">{{ $paymentVoucher->voucher_number }}</td>
                            </tr>
                            <tr>
                                <td
                                    class="font-semibold {{ $printLang == 'ar' ? 'pl-3 text-left' : 'pr-3 text-right' }}">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="font-bold border border-black px-1.5 py-0.5 inline-block text-8">
                                        {{ strtoupper($paymentVoucher->payment_method_label) }}
                                    </span>
                                </td>
                            </tr>
                            @if ($paymentVoucher->reference_number)
                                <tr>
                                    ...
                                    <td>{{ $paymentVoucher->reference_number }}</td>
                                </tr>
                            @endif
                        </table>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div class="border border-black px-2 py-1.5">
                        <p class="text-8 font-bold uppercase mb-0.5">
                            {{ $printLang == 'ar' ? 'دُفع إلى' : 'PAID TO' }}
                        </p>
                        <p class="text-10 font-bold mb-0.5">{{ $paymentVoucher->party->name }}</p>
                        <div class="text-8">
                            @if ($paymentVoucher->party->address)
                                <p>{{ $paymentVoucher->party->address }}</p>
                            @endif
                            <p>
                                @if ($paymentVoucher->party->phone)
                                    <span>{{ $printLang == 'ar' ? 'ت:' : 'T:' }}
                                        {{ $paymentVoucher->party->phone }}</span>
                                @endif
                                @if ($paymentVoucher->party->email)
                                    <span class="ml-2">{{ $paymentVoucher->party->email }}</span>
                                @endif
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="border border-black px-2 py-1.5">
                        <p class="text-8 font-bold uppercase mb-0.5">
                            {{ $printLang == 'ar' ? 'مسحوب من حساب' : 'PAID FROM' }}
                        </p>
                        <p class="text-10 font-bold mb-0.5">{{ $paymentVoucher->account->account_name }}</p>
                        <div class="text-8">
                            @if ($paymentVoucher->account->account_number)
                                <p>{{ $printLang == 'ar' ? 'رقم الحساب:' : 'Account No:' }}
                                    {{ $paymentVoucher->account->account_number }}</p>
                            @endif
                            @if ($paymentVoucher->account->bank_name)
                                <p>{{ $paymentVoucher->account->bank_name }}</p>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-9">
                            ...
                            <tr>
                                ...
                                <td class="border-all px-2 py-1 text-center">
                                    {{ $paymentVoucher->purchaseInvoice->invoice_date->format('d/m/Y') }}
                                </td>
                                <td class="border-all px-2 py-1 text-{{ $printLang == 'ar' ? 'left' : 'right' }}">
                                    {{ number_format($paymentVoucher->purchaseInvoice->total_amount, 2) }}
                                </td>
                                <td class="border-all px-2 py-1 text-{{ $printLang == 'ar' ? 'left' : 'right' }}">
                                    {{ number_format($paymentVoucher->purchaseInvoice->paid_amount, 2) }}
                                </td>
                                <td
                                    class="border-all px-2 py-1 text-{{ $printLang == 'ar' ? 'left' : 'right' }} font-bold">
                                    {{ number_format($paymentVoucher->purchaseInvoice->outstanding_balance, 2) }}
                                </td>
                            </tr>
                        </table>
                    </div>
                ...
                {{-- ══ DESCRIPTION / NOTES ══════════════════════════════ --}}
                @if ($paymentVoucher->description || $paymentVoucher->notes)
