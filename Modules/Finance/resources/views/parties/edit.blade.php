<x-dashboard :pageTitle="__('finance::party.edit_party')">
    <div class="max-w-5xl mx-auto">
        ...
        <nav class="mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                ...
                <li>
                    <span class="text-gray-900 font-medium">{{ __('finance::party.edit_party') }}</span>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-900">{{ __('finance::party.edit_party') }}: {{ $party->name }}</h2>
                <p class="text-sm text-gray-600 mt-1">{{ __('common.required_fields') }}</p>
            </div>

            <div class="p-6">
                <x-dashboard.packages.form-builder 
                    :action="route('finance.parties.update', $party)" 
                    method="POST"
                    :formConfig="[
                        'groups' => [
                            [
