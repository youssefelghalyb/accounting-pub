<x-dashboard :pageTitle="__('finance::invoice.invoice_management')">
        ...
        $tableColumns = [
            ...
            [
                ...
                'render' => function($row) {
                    ...
                    return '<span class="font-medium ' . $color . '">' . number_format($row['outstanding_balance'], 2) . '</span>';
                }
            ],
            [
                ...
                'render' => function($row) {
                    ...
                    $html .= '<span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-' . $row['status_color'] . '-100 text-' . $row['status_color'] . '-800">' . e($row['status_label']) . '</span>';
                    if ($row['is_overdue']) {
                        $html .= '<span class="text-xs text-red-600">' . __('finance::invoice.days_overdue', ['days' => $row['days_overdue']]) . '</span>';
                    }
                    $html .= '</div>';
                    return $html;
                }
    ...
    <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">{{ $stats['total_invoices'] }} {{ __('finance::invoice.sales_invoices') }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900 mt-2">{{ number_format($stats['total_outstanding'], 2) }}</p>
                    </div>
        ...
        <x-dashboard.packages.data-table
            :title="__('finance::invoice.invoice_list')"
            :description="__('finance::invoice.total_invoices') . ': ' . $invoices->count()"
            searchable
            :searchRoute="route('finance.sales-invoices.index')"
            :searchPlaceholder="__('finance::invoice.search_placeholder')"
            :filters="$tableFilters"
            :data="$tableData"
            :columns="$tableColumns"
            :actions="$tableActions"
            :createRoute="route('finance.sales-invoices.create')"
            :createLabel="__('finance::invoice.add_invoice')"
            :emptyStateTitle="__('finance::invoice.no_invoices')"
            :emptyStateDescription="__('common.no_data')"
            emptyStateIcon="document-text"
        />
    </div>
</x-dashboard>