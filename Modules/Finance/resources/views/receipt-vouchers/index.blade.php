<x-dashboard :pageTitle="__('finance::receipt.receipt_management')">
    @php
        ...
        $tableData = $receipts
            ->map(function ($receipt) {
                return [
                    ...
                    'description' => \Illuminate\Support\Str::words($receipt->description, 5, '...'),
        ...
        $tableColumns = [
            ...
            [
                'label' => __('finance::receipt.description'),
                'field' => 'description',
                'format' => function ($value) {
                    return '<span class="text-sm text-gray-900">' . e($value) . '</span>';
                },
            ...
            [
                ...
                'format' => function ($value) {
                    return $value
                        ? '<span class="text-sm text-blue-600">' . e($value) . '</span>'
            ...
            [
                ...
                'render' => function ($row) {
                    $html =
                        ...
                        $row['payment_method_color'] .
                        '-100 text-' .
                        $row['payment_method_color'] .
                        '-800">' .
                        e($row['payment_method_label']) .
                        '</span>';
    ...
    <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900 mt-2">{{ number_format($stats['total_amount'], 2) }}
                        </p>
                        <p class="text-xs text-gray-500 mt-1">{{ $stats['total_receipts'] }}
                            {{ __('finance::receipt.receipt_vouchers') }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-blue-600 mt-2">
                            {{ number_format($stats['today_receipts'], 2) }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-purple-600 mt-2">
                            {{ number_format($stats['this_month_receipts'], 2) }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-emerald-600 mt-2">
                            {{ number_format($stats['cash_receipts'], 2) }}</p>
                    </div>
        ...
        <!-- DataTable -->
        <x-dashboard.packages.data-table :title="__('finance::receipt.receipt_list')" :description="__('finance::receipt.total_receipts') . ': ' . $receipts->count()" searchable :searchRoute="route('finance.receipt-vouchers.index')"
            :searchPlaceholder="__('finance::receipt.search_placeholder')" :filters="$tableFilters" :data="$tableData" :columns="$tableColumns" :actions="$tableActions" :createRoute="route('finance.receipt-vouchers.create')"
            :createLabel="__('finance::receipt.add_receipt')" :emptyStateTitle="__('finance::receipt.no_receipts')" :emptyStateDescription="__('common.no_data')" emptyStateIcon="receipt-refund" :pagination="$receipts"
