...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                @if ($searchable && $searchRoute)
                    <form method="GET" action="{{ $searchRoute }}" class="flex gap-2">
                        ...
                        <div class="relative">
                            ...
                            <button type="submit"
                                class="absolute {{ $isRtl ? 'left-3' : 'right-3' }} top-2.5 text-gray-400 hover:text-gray-600">
                        ...
                        <form method="GET" action="{{ $searchRoute }}" class="flex gap-2">
                            ...
                            @foreach ($filters as $otherFilter)
                                @if ($otherFilter['name'] !== $filter['name'] && request($otherFilter['name']))
                                    <input type="hidden" name="{{ $otherFilter['name'] }}"
                                        value="{{ request($otherFilter['name']) }}">
                                @endif
                            @endforeach
    ...
    <div class="overflow-x-auto">
        @if (count($data) > 0)
                ...
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        @foreach ($columns as $column)
                            <th
                                class="px-6 py-3 text-{{ $column['align'] ?? 'start' }} text-xs font-semibold text-gray-600 uppercase tracking-wider {{ $column['class'] ?? '' }}">
                                {{ $column['label'] }}
                            </th>
                        @endforeach
                            ...
                            @if (count($bulkActions) > 0)
                                <td class="px-6 py-4 text-center">
                                    <input type="checkbox"
                                        ...
                                        value="{{ $row['id'] ?? $index }}" onchange="updateBulkActionsBar()">
                                </td>
                            @endif

                            @foreach ($columns as $column)
                                <td
                                    class="px-6 py-4 {{ $column['nowrap'] ?? true ? 'whitespace-nowrap' : '' }} text-{{ $column['align'] ?? 'start' }}">
                                    @php
                                        ...
                                        @foreach ($actions as $action)
                                            ...
                                            @if ($show)
                                                ...
                                                @elseif($action['type'] === 'form')
                                                    <form action="{{ $action['route']($row) }}" method="POST"
                                                        class="inline-block"
            ...
            <!-- Pagination -->
{{-- REMOVE the old pagination block entirely and replace with: --}}
@if ($pagination)
    <div class="px-6 py-4 border-t border-gray-200">
        <x-dashboard.packages.pagination
