...
<body class="bg-gray-50 font-sans antialiased">
    <div class="flex h-screen overflow-hidden">
        <!-- Sidebar -->
        <aside id="sidebar" class="sidebar fixed inset-y-0 {{ app()->getLocale() == 'ar' ? 'right-0' : 'left-0' }} z-50 bg-white shadow-xl md:static md:translate-x-0">
            <div class="flex h-full flex-col">
                ...
                <div class="flex h-16 items-center justify-between border-b border-gray-200 px-4">
                    <div class="flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} overflow-hidden">
                        ...
                        @else
                            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-primary text-white font-bold text-lg flex-shrink-0">
                                {{ $orgSettings ? strtoupper(substr($orgSettings->organization_name ?? __('sidebar.organization_name'), 0, 1)) : 'D' }}
                            </div>
                        @endif
                        <span class="org-name text-lg font-bold text-gray-800 truncate">{{ $orgSettings->organization_name ?? __('sidebar.organization_name') }}</span>
                    </div>
                ...
                <nav class="flex-1 overflow-y-auto custom-scrollbar px-3 py-4">
                    ...
                    @else
                        ...
                        <div class="group-header">
                            ...
                            <span class="group-title text-xs font-semibold text-gray-500 uppercase tracking-wider {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('sidebar.main_menu') }}</span>
                            <div class="group-divider"></div>
                        </div>

                        <div class="space-y-1">
                            <a href="{{ route('dashboard') }}" class="sidebar-item flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} rounded-lg px-3 py-2.5 text-gray-700 hover:bg-gray-100 {{ request()->routeIs('dashboard') ? 'active bg-gray-100' : '' }}">
                            ...
                            <a href="#" class="sidebar-item flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} rounded-lg px-3 py-2.5 text-gray-700 hover:bg-gray-100">
                                <svg class="h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="#" class="sidebar-item flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} rounded-lg px-3 py-2.5 text-gray-700 hover:bg-gray-100">
                                ...
                                <span class="sidebar-text font-medium">{{ __('sidebar.projects') }}</span>
                                <span class="sidebar-tooltip">{{ __('sidebar.projects') }}</span>
                            </a>
                        </div>
                        ...
                        <div class="group-header">
                            ...
                            <span class="group-title text-xs font-semibold text-gray-500 uppercase tracking-wider {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('sidebar.management') }}</span>
                            <div class="group-divider"></div>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <a href="#" class="sidebar-item flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} rounded-lg px-3 py-2.5 text-gray-700 hover:bg-gray-100">
                                ...
                                <span class="sidebar-text font-medium">{{ __('sidebar.documents') }}</span>
                                <span class="sidebar-tooltip">{{ __('sidebar.documents') }}</span>
                            </a>
                        ...
                        <div class="group-header">
                            ...
                            <span class="group-title text-xs font-semibold text-gray-500 uppercase tracking-wider {{ app()->getLocale() == 'ar' ? 'mr-3' : 'ml-3' }}">{{ __('sidebar.support') }}</span>
                            <div class="group-divider"></div>
                        </div>
                ...
                <div class="border-t border-gray-200 p-4">
                    <!-- Toggle Button -->
                    <button id="toggleSidebar" class="hidden md:flex items-center justify-center w-full mb-3 px-3 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                        <svg class="h-5 w-5 chevron-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"></path>
                        </svg>
                        <span class="sidebar-text {{ app()->getLocale() == 'ar' ? 'mr-2' : 'ml-2' }}">{{ __('sidebar.collapse') }}</span>
                    </button>
                    ...
                    <div class="flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                        ...
                        <div class="user-info flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ auth()->user()->name ?? 'User Name' }}</p>
                            <p class="text-xs text-gray-500 truncate">{{ auth()->user()->email ?? 'user@example.com' }}</p>
                        </div>
                        <form method="POST" action="{{ route('logout') }}" class="flex-shrink-0">
                            @csrf
                            <button type="submit" class="text-gray-400 hover:text-gray-600" title="{{ __('sidebar.logout') }}">
            ...
            <header class="bg-white shadow-sm">
                <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex items-center space-x-4 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                        ...
                        @if($orgSettings && $orgSettings->logo_path)
                            <div class="hidden mx-2 sm:flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} px-3 py-1.5 bg-gray-50 rounded-lg border border-gray-200">
                                <img src="{{ asset('storage/' . $orgSettings->logo_path) }}" 
                                     alt="{{ $orgSettings->organization_name }}" 
                                     class="h-6 w-6 object-contain">
                                <span class="text-sm font-medium text-gray-700">{{ $orgSettings->organization_name }}</span>
                            </div>
                        @endif
                        ...
                        <button class="relative text-gray-500 hover:text-gray-700">
                            ...
                            <span class="absolute top-0 {{ app()->getLocale() == 'ar' ? 'left-0' : 'right-0' }} block h-2 w-2 rounded-full bg-red-500 ring-2 ring-white"></span>
                        </button>

                        <!-- User Menu -->
                        <div class="relative">
                            <button class="flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} text-gray-700 hover:text-gray-900">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'User') }}&background=random" 
                                     alt="User" 
